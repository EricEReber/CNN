...
def DFT(vec): 
    ...
    for u in range(N): 
        for x in range(N): 
            fourier_1d[u] += vec[x] * (np.cos((2*np.pi*u*x/N)) - (1j*np.sin((2*np.pi*u*x/N))))
...
def FFT(vec): 
    ...
    if N == 1: 
        ...
        vec_even = FFT(vec[::2])
        vec_odd = FFT(vec[1::2])
        W_u_2k = np.cos(2*np.pi*np.arange(N)/N) - 1j*np.sin(2*np.pi*np.arange(N)/N)
...
def log_transform(fourier_img): 
    ...
    for i in range(fourier_img.shape[0]):
        for j in range(fourier_img.shape[1]): 
            fourier_spectrum[i,j] = np.sqrt(fourier_img[i,j].real**2 + fourier_img[i,j].imag**2)

    # Log transform of image
    fourier_img = (255/np.log10(1 + 255)) * np.log10(1 + (255/(np.max(fourier_spectrum))*fourier_spectrum))
