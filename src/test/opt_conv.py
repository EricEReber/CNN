...
def _padding(batch, kernel_size):
    ...
    new_tensor = np.ndarray((batch.shape[0], batch.shape[1], new_height, new_width))
...
def _extract_windows(batch, kernel_size, stride=1):
    ...
    for h in range(img_height - kernel_size + 1):
        for w in range(img_width - kernel_size + 1):
...
if __name__ == "__main__":
    ...
    images = np.ndarray((3, image.shape[2], image.shape[0], image.shape[1]))
    ...
    for x in range(images.shape[0]):
        images[x, :, :, :] = image[:, :, :].transpose(2, 0, 1)

    out_grad = np.ndarray((3, out_channels, image.shape[0], image.shape[1]))
    ...
    out = _backpropagate(images, params, out_grad)
